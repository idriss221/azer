var CLE_STOCKAGE = "baseDonneesMiamGo";

function afficherMessage(id, texte, type) {
  var boite = document.getElementById(id);

  if (!boite) return;

  boite.textContent = texte;
  boite.style.display = "block";
  boite.style.padding = "0.6vh 1vh";
  boite.style.borderRadius = "10px";
  boite.style.fontWeight = "600";

  if (type === "error") {
    boite.style.background = "rgba(120, 20, 20, 0.18)";
    boite.style.color = "#ffdede";
    boite.style.border = "1px solid rgba(255,180,180,0.35)";
  } else {
    boite.style.background = "rgba(18, 95, 36, 0.18)";
    boite.style.color = "#e6ffe8";
    boite.style.border = "1px solid rgba(150,255,180,0.35)";
  }
}
...
function lireComptes() {
  var texte = localStorage.getItem(CLE_STOCKAGE);
...
if (formulaireInscription) {
  formulaireInscription.addEventListener("submit", function(e) {
    ...
    if (verifierEmail(email) === false) {
      afficherMessage("msgInscription", "L'e-mail doit contenir @ et un point.", "error");
      return;
    }

    var comptes = lireComptes();

    var emailDejaUtilise = false;
    ...
    afficherMessage("msgInscription", "Compte créé. Vous pouvez vous connecter.", "success");
...
if (formulaireConnexion) {
  formulaireConnexion.addEventListener("submit", function(e) {
    ...
    if (email === "" || motDePasse === "") {
      afficherMessage("msgConnexion", "Écrivez votre e-mail et votre mot de passe.", "error");
      return;
    }
    ...
    afficherMessage("msgConnexion", "Connexion réussie. Redirection…", "success");
    ...
    setTimeout(function() {
      ...
      var prenom = compte.prenom || email.split("@")[0];
      var nomNav = document.getElementById("welcomeName");
      if (nomNav) {
        nomNav.textContent = "Bonjour, " + prenom;
